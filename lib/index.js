...
/**
 * Enum of string or number literals
 * 
 * @template {string | number} TStringOrNumber
 * @template {[TStringOrNumber, TStringOrNumber, ...TStringOrNumber[]]} TLiteralValue 
 * 
 * @param {TLiteralValue} stringOrNumberLiteralsArray 
 */
function literalEnum(stringOrNumberLiteralsArray) {
  const literalA = literal(stringOrNumberLiteralsArray[0]);
  const literalB = literal(stringOrNumberLiteralsArray[1]);
  const literalsRest = stringOrNumberLiteralsArray.slice(2).map(literal);

  const ioTsTypeA = literalA._ioTsType;
  const ioTsTypeB = literalB._ioTsType;
  const ioTsTypesRest = literalsRest.map(s => s._ioTsType);
  const ioTsType = t.union([ioTsTypeA, ioTsTypeB, ...ioTsTypesRest]);
  
  const fastCheckArbitrary = fc.oneof(...([literalA, literalB, ...literalsRest].map(speck => speck._fastCheckArbitrary)));
  return createObjectSpeck(ioTsType, fastCheckArbitrary);
}
