...
const { ioTsJsDate } = require('./ioTsTypes');
 ...
 */
const jsDate = createNonObjectSpeck(ioTsJsDate, fc.date());

/**
 * `null` or `undefined`
 */
const nil = createNonObjectSpeck(
...
function record(keySpeck, valueSpeck) {
  ...
  return /** @type {any} */(createObjectSpeck(
    t.record(keySpeck._ioTsType, valueSpeck._ioTsType),
    fc.dictionary(keySpeck._fastCheckArbitrary, valueSpeck._fastCheckArbitrary)));
}
