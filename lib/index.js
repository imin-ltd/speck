...
/**
 * A speck that could be one non-object type or another. Equivalent to `|` in TypeScript 
 * (https://www.typescriptlang.org/docs/handbook/advanced-types.html#union-types).
 *
 * This function is defined specifically for non-objects. This means that it is
 * not possible to use an object speck as one of the parameters. This means
 * that its return value will certainly be a non-object speck.
 *
 * This is important as intersection() cannot meaningfully work on
 ...
 * ```js
 * >  var s = require('./lib')
 * > const NumberOrString = s.unionNonObjects([s.string, s.int]);
 ...
 * ```
 *
 * @template {import('./types').NonObjectSpeck<any>} TA
 * @template {import('./types').NonObjectSpeck<any>} TB
 * @param {[TA, TB]} specks
 * @returns {import('./types').UnionNonObjectsType<[TA, TB]>}
 */
function unionNonObjects([speckA, speckB]) {
  const ioTsType = t.union([speckA._ioTsType, speckB._ioTsType]);
  const fastCheckArbitrary = fc.oneof(speckA._fastCheckArbitrary, speckB._fastCheckArbitrary);
  return createNonObjectSpeck(ioTsType, fastCheckArbitrary);
}
